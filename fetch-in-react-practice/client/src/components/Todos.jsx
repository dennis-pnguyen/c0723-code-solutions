...
export default function Todos() {
  ...
  async function addTodo(newTodo) {
    try {
      ...
      if (!response.ok)
        throw new Error(`Network response was not OK:', ${response.status}`);
      const newItemTodo = await response.json();
      setTodos(todos.concat(newItemTodo));
    } catch (error) {
      console.error(error.message);
      setError(error);
    }
  ...
  async function toggleCompleted(todoId) {
    const todoObjIndex = todos.findIndex((x) => x.todoId === todoId);
    const todoObj = todos[todoObjIndex].isCompleted;
    const todoObjStatus = { isCompleted: !todoObj };
    try {
      const response = await fetch(`/api/todos/${todoId}`, {
        ...
        body: JSON.stringify(todoObjStatus),
      });
      if (!response.ok)
        throw new Error(`Network response was not OK:', ${response.status}`);
      ...
      newTodosArr[todoObjIndex] = newTodos;
